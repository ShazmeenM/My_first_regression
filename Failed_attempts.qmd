...
# grouping rows (LA_Name) by columns (Year) for each column separately
group = housing_sale_price.groupby(['LA_Name', 'Year'])[['Detached', 'Semi-detached', 'Terraced', 'Flats']].first()
print(group)
#group_semi-detached = housing_sale_price.groupby(['LA_Name', 'Year'])['Semi-detached'].first()
#group_terraced = housing_sale_price.groupby(['LA_Name', 'Year'])['Terraced'].first()
#group_flats = housing_sale_price.groupby(['LA_Name', 'Year'])['Flats'].first()

# incomplete try
housing_sale_price['value_diff'] = housing_sale_price.groupby('LA_Name', 'Year').value.diff()
housing_sale_price.groupby('LA_Name', 'Year').housing_sale_price['Detached'].diff()


# Filter all rows for which the year is less than 2001
#group_filtered = group[group['Year'] < 2001]
 
# Print the new dataframe
#print(group_filtered)
 

for p in set(group['Detached']):
    max = group.loc[group['Detached'] == p,'London']
    min = df.loc[df['product'] == p,'value2']
    if  max_ > min_:
...
if x > group['LA_Name'] == 'London':


for x in group['Detached']:
  for y in group['LA_Name']:
    if x > "London":
       continue
    print(y)
...
housing_sale_price_regions.loc['Detached', 'London']

group['Price'] = [0 if x =='London' else 800 for x in df['LA_Name']]

n = group[group['LA_Name'].isin(['London']) ]
y = group
...
n = group[group['LA_Name'].isin(['London']) ]
y = group[-group['LA_Name'].isin(['London']) ]
for i, value in enumerate(n):
