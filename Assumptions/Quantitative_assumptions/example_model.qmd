...
group = housing_sale_price_regions.groupby(['LA_Name', 'Year'], as_index=False)[['Detached', 'Semi-detached', 'Terraced', 'Flats']].first()

#print(group)

# Using DataFrame.query() method extract column values.
df=group[group['LA_Name'] == 'London']['Detached'] < group[group['LA_Name'] == 'Wales']
#print(df2)

LA_lon = group[group['LA_Name'].isin(['London']) ]
LA = group[-group['LA_Name'].isin(['London']) ]
for i in LA_lon:
  for p in LA:
    if i < p:
        print (LA)
